...
gaussian_kernel <- function(x, x_data, h) {
  exp(-0.5*(x - x_data)^2 / h^2) / (sqrt(2 * pi) * h)
}
...
for (i in 1:n_rep) {
  ...
  b_temp_hat <- function(x) {
    weights <- gaussian_kernel(x, x_q, h)
    result <- mean(weights * (theta_p1_hat - y0_q)) / mean(weights)
    
    ifelse(is.nan(result), 0, result)
  }
  
  b_temp_p <- sapply(x_p,b_temp_hat)
  b_temp_q <- sapply(x_q,b_temp_hat)
  rho_q <- sapply(x_q,rho_hat)
  
  theta_hat <- mean(y1_p - b_temp_p) + mean((-y0_q + b_temp_q) * rho_q)
  
  theta_p0_hat <- theta_p1_hat - theta_hat
