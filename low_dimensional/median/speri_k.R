...
for (i in 1:n_rep) {
  ...
  influence_function <- function(r,y,x) {
    r/pi_p*(B0*b_hat(x,h)-B1*((y <= theta_p1_hat)-0.5))+
      (1-r)/(1-pi_p)*B0*(((y <= theta_p0_hat)-0.5)-b_hat(x,h))*rho_hat(x,h)
  }
