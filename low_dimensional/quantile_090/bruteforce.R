...
for (i in 1:n_rep) {
  ...
  p0_func <- function(theta_p0,x,y0) {
    ((y0 <= theta_p0)-tau)*rho_hat(x)
  }
  
  p0_sum <- function(theta_p0,x,y0) {
    result <- mapply(p0_func,x=x,y0=y0,MoreArgs=list(theta_p0=theta_p0))
    sum(result)
  }
  
  theta_p0_hat <- uniroot(p0_sum,c(0,2),x=x_q,y0=y0_q)$root
  ...
  influence_function <- function(r,y,x) {
    r/pi_p*(B0*b_hat(x)-B1*((y <= theta_p1_hat)-tau))+
      (1-r)/(1-pi_p)*B0*(((y <= theta_p0_hat)-tau)-b_hat(x))*rho_hat(x)
  }
