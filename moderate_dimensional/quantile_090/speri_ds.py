...
def fit_net(net, X, y, loss_fn, n_epochs=600, lr=1e-5, batch_size=256,
            val_frac=0.2, patience=10, min_delta=1e-6, min_epochs=100):
...
for rep in range(n_rep):
    ...
    # Estimate conditional 0.9 quantile
    ...
    # Full-data nuisance estimation for variance

    rho_full_net = fit_net(
        build_classifier(d),
        np.vstack([x_p, x_q]),
        np.hstack([np.ones(n_p), np.zeros(n_q)])[:, None],
        mse
    )

    @torch.no_grad()
    def rho_full(x):
        p = rho_full_net(torch.as_tensor(x, dtype=torch.float32)).numpy().squeeze()
        return p/(1-p+1e-8)
    ...
    @torch.no_grad()
    def b_full(theta, x):
        quantile = b_full_net(torch.as_tensor(x, dtype=torch.float32)).numpy().squeeze()
        return (theta < quantile).astype(float)-tau
    ...
    try:
        theta_1 = brentq(eff1, -1, 4)
        theta_2 = brentq(eff2, -1, 4)
    ...
    rho_hat = rho_full(x_all)
    b_hat = b_full(theta_p0_hat, x_all)
