...
for (b in 1:B) {
  ...
  if (is.na(score.left) ||
    ...
    theta.p0.hat <- matrixStats::weightedMedian(
      y0.q,
      w = rho.q
    )
    ...
    theta.p0.hat <- uniroot(
      function(theta) score(theta),
      interval = bracket
    )$root
  }
  ...
  b.sign <- function(x) {
    ...
    wq <- prod.kernel.weights(
      x[2:4], Xq.g[, 2:4, drop = FALSE], h
    )
    
    z <- as.numeric((y0.g <= theta.p0.hat)-0.5)
