...
for (b in 1:B) {
  ...
  b.point <- function(theta,x) {
    ...
    wq <- prod.kernel.weights(x[2:4],Xq.g[,2:4,drop=FALSE],h)
    z <- as.numeric((y0.g<=theta)-tau)
    
    num <- mean(wq*z)+sign(mean(wq*z))*1e-4
    den <- mean(wq)+sign(mean(wq))*1e-4
  ...
  if (!is.finite(score.left) || !is.finite(score.right) ||
  ...
  # Conditional quantile score for variance estimation
