...
def fit_net(model, X, y, loss_fn, lr):
    ...
    loader = DataLoader(TensorDataset(X[train_idx], y[train_idx]), shuffle=True, batch_size=BATCH_SIZE)
    ...
    for epoch in range(EPOCHS):
        ...
        model.eval()
        with torch.no_grad():
            val_loss = loss_fn(model(X[val_idx].to(device)), y[val_idx].to(device)).item()
...
def find_root(fun, lower, upper):
    ...
    result = minimize_scalar(lambda x: fun(x)**2, bounds=(lower, upper), method="bounded",
...
def run_sim(seed):
    ...
    b1_model = fit_net(build_quantile_net(), x_q[q1], y0_q[q1, None], qloss, LR_Q)
    b2_model = fit_net(build_quantile_net(), x_q[q2], y0_q[q2, None], qloss, LR_Q)
    ...
    lower = theta_p1-1.14-EE_BRACKET_HALF
    upper = theta_p1-1.14+EE_BRACKET_HALF
